"""CLI: ``python -m veriworld.benchmark.interactive.navigation.mazenavfps.pv_bf``."""
...
def _args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="MazeNavFPS PV/Bf (pure vision) ablation")
    ...
    p.add_argument("--history-size", type=int, default=6)
...
async def _amain() -> None:
    ...
    try:
        result = await run(
            ...
            max_steps=a.max_steps, history_size=a.history_size,
            initial_yaw=a.initial_yaw,
        )
        print(result)
    finally:
        await engine.stop()
